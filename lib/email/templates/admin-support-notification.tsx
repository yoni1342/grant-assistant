...
import type { AdminSupportNotificationEmailParams } from '../types'

export default function AdminSupportNotificationEmail({
  fullName,
  ticketRef,
  subject,
  preview,
  ...
  const heading =
    kind === 'customer_reply' ? 'Customer replied to a support thread' : 'New support request'

  return (
    <EmailLayout preview={`[${ticketRef}] ${subject}`}>
      <Heading style={h1}>{heading}</Heading>

      <Text style={meta}>Hi {fullName}, there&apos;s new activity in the support inbox.</Text>
      ...
      <Text style={replyHint}>
        Reply from the admin panel so the response is logged to this thread.
      </Text>
    </EmailLayout>
  )
}
